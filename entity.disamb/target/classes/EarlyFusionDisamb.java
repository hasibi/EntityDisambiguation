...
/**
 * This code is tested with weka using the same training and test set. 
 * The cross validation part is tested in TestValidation class.
 * @author faeghehhasibi
 *
 */
public class EarlyFusionDisamb extends Disamb {
	
	public EarlyFusionDisamb(Wikis wikis, Geos geos, int[] gains){
	...
	public void disambiguate(String outputFile){
		...
		addPCAWikiPop(cpoyWikis, cpoyWikis, "pca/wikis", true);
		...
		IO.writeToFile(evalStr, outputFile + ".evel.txt");
		...
		IO.writeData(mergedEns, outputFile + ".preds");
	}
	...
	private String crossValidate(ArrayList<String> shuffledNames, int folds) throws Exception{
		...
		for (int i = 0; i < folds; i++){
			...
			addPCAWikiPop(trainWiki, testWiki, "pca/wikis.CV"+i, false);
	 ...
	 */
	private void addPCAWikiPop(Wikis trainSet, Wikis testSet, String setLabel, boolean sameTrainTest){
